      ...
      <div className="pd-sparkline">
        {daily.map(d => {
          ...
          const day = d.date_id
            ...
            : '';
          return (
            <div key={d.date_id} className="pd-bar-col" title={`${d.date_id}: ${d.minutes > 0 ? fmtDur(d.minutes) : 'no data'}`}>
              <div className="pd-bar-track">
                <div className="pd-bar-fill" style={{ height: `${Math.max(h, 2)}%`, background: d.minutes > 0 ? bg : 'rgba(255,255,255,0.06)' }} />
                {goal > 0 && <div className="pd-goal-line" style={{ bottom: `${Math.min((goal / max) * 100, 98)}%` }} />}
              </div>
...
function NextSessionPanel({ session }) {
  ...
  const relative = dayDiff === 0 ? 'Today' : dayDiff === 1 ? 'Tomorrow' : dayDiff > 1 ? `In ${dayDiff} days` : 'Past';
  ...
  return (
    <div className="dashboard-panel">
      ...
      <div className="pd-session-hero">
        ...
        <div className="pd-session-meta">{dateLabel} · {session.time_slot} · {fmtDur(session.duration_mins)}</div>
        {session.notes && <p className="pd-session-notes">{session.notes}</p>}
      </div>
    </div>
  );
}
      ...
      {build === 0 && quit === 0 ? (
        ...
        <div className="pd-habit-grid">
          <div className="pd-habit-block">
            ...
            <div className="pd-habit-bar-track"><div className="pd-habit-bar-fill" style={{ width: `${wc.build_pct ?? 0}%`, background: '#2ECC71' }} /></div>
          ...
          <div className="pd-habit-block">
            ...
            <div className="pd-habit-bar-track"><div className="pd-habit-bar-fill" style={{ width: `${wc.quit_pct ?? 0}%`, background: '#9B59B6' }} /></div>
  ...
  return (
    <div className="dashboard-panel">
      ...
      {!hasEntry ? (
        ...
        <>
          <div className="tj-dash-last-row">
            ...
            {lastEntry.state?.mood != null && <span className="tj-dash-mood">Mood {lastEntry.state.mood}/10</span>}
            {lastEntry.pattern_count > 0 && <span className="tj-dash-chip">{lastEntry.pattern_count} pattern{lastEntry.pattern_count !== 1 ? 's' : ''}</span>}
            {lastEntry.open_question_count > 0 && <span className="tj-dash-chip">{lastEntry.open_question_count} open Q</span>}
          </div>
          {openGoals?.length > 0 && (
            <>
              ...
              <ul className="tj-dash-goals">
                {openGoals.map(g => (
                  <li key={g.id} className="tj-dash-goal-item">
                    <span className="tj-dash-goal-icon" style={{ color: GOAL_STATUS_COLORS[g.status] }}>{GOAL_STATUS_ICONS[g.status] || '○'}</span>
      ...
      <div className="tj-dash-cta">
        <button className="tj-dash-cta-primary" onClick={onPrepare}>Prepare for session ↗</button>
      </div>
    </div>
  );
}
    ...
    <div className="dashboard-panel">
      ...
      {projects.length === 0 ? (
        ...
        <div className="project-progress-list">
          {projects.map(p => {
            const pct = p.total_tasks > 0 ? Math.round((p.complete_tasks / p.total_tasks) * 100) : 0;
            return (
              <Link key={p.id} to={`/projects/${p.id}`} className="project-progress-card pd-project-card">
                <div className="proj-card-top">
                  ...
                  <span className={`proj-card-badge ${(p.phase || 'plan').toLowerCase()}-phase`}>{p.phase}</span>
                </div>
                <div className="proj-progress-stats"><span>Tasks</span><span>{p.complete_tasks} / {p.total_tasks} ({pct}%)</span></div>
                <div className="proj-progress-bar-container">
                  <div className="proj-progress-bar-fill" style={{
                    width: `${pct}%`,
                    background: pct === 100 ? 'linear-gradient(90deg,#2ECC71,#27AE60)' : `linear-gradient(90deg,${ACCENT},#C0395E)`,
                  }} />
                </div>
...
function saveScopeLabels(labels) { localStorage.setItem(STORAGE_KEY, JSON.stringify(labels)); }
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '7px 0', borderBottom: '1px solid var(--border)' }}>
      {editing ? (
        <input ref={inputRef} value={draft} onChange={e => setDraft(e.target.value)} onBlur={save}
          ...
          style={{ flex: 1, fontSize: '13px', background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: '4px', padding: '4px 8px', color: 'var(--text-primary)', outline: 'none' }}
        />
      ) : (
        <Link to={`/personal-care/goals/${goal.id}`} onDoubleClick={() => { setDraft(goal.title); setEditing(true); }}
          style={{ flex: 1, fontSize: '13px', color: 'var(--text-primary)', textDecoration: 'none', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={goal.title}>
          {goal.title}
        </Link>
      )}
      {goal.status === 'completed' && <span style={{ fontSize: '11px', color: '#2ECC71', fontWeight: 600, flexShrink: 0 }}>Done</span>}
      <button onClick={() => onArchive(goal.id)} title="Archive"
        style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-dimmed)', fontSize: '13px', padding: '0 2px', flexShrink: 0, lineHeight: 1 }}>
        ⊠
      </button>
    </div>
  );
...
function GoalsWidget({ scope, goals, onAdd, onArchive, onUpdate, onRename, scopeLabel, loading }) {
  ...
  async function handleAdd(e) {
    ...
    setSaving(true);
    try { await onAdd(scope, draft.trim()); setDraft(''); setAddOpen(false); }
    ...
    <div className="dashboard-panel" style={{ borderTop: `2px solid ${meta.color}44` }}>
        ...
        <div>
          {editingTitle ? (
            <input autoFocus value={titleDraft} onChange={e => setTitleDraft(e.target.value)}
              onBlur={() => { const t = titleDraft.trim(); if (t && t !== label) onRename(scope, t); setEditingTitle(false); }}
              onKeyDown={e => { if (e.key === 'Escape') { const t = titleDraft.trim(); if (t && t !== label) onRename(scope, t); setEditingTitle(false); }}}
              style={{ fontSize: '14px', fontWeight: 700, background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: '4px', padding: '3px 8px', color: 'var(--text-primary)', outline: 'none', width: '100%' }}
            />
          ) : (
            <h3 className="panel-title" style={{ color: meta.color, cursor: 'pointer' }} onDoubleClick={() => { setTitleDraft(label); setEditingTitle(true); }} title="Double-click to rename">
              {meta.icon} {label}
            </h3>
          )}
          <p className="panel-subtitle">
            {activeGoals.length} active goal{activeGoals.length !== 1 ? 's' : ''}{' · '}
            <span onClick={() => setShowCompleted(v => !v)} style={{ cursor: 'pointer', textDecoration: 'underline dotted', color: showCompleted ? '#2ECC71' : 'var(--text-dimmed)' }}>
              {showCompleted ? 'hiding completed' : `${goals.filter(g => g.status === 'completed').length} completed`}
            </span>
          </p>
        </div>
        <button className="pd-panel-link" onClick={() => setAddOpen(v => !v)}
      ...
      {addOpen && (
        <form onSubmit={handleAdd} style={{ display: 'flex', gap: '6px', marginBottom: '10px' }}>
          <input autoFocus placeholder="Goal title…" value={draft} onChange={e => setDraft(e.target.value)} onKeyDown={e => e.key === 'Escape' && setAddOpen(false)}
            style={{ flex: 1, background: 'var(--surface-2)', border: `1px solid ${meta.color}66`, borderRadius: '5px', padding: '5px 10px', fontSize: '13px', color: 'var(--text-primary)' }} />
          <button type="submit" disabled={saving || !draft.trim()}
            style={{ background: meta.color, color: '#fff', border: 'none', borderRadius: '5px', padding: '5px 14px', fontSize: '12px', fontWeight: 600, cursor: 'pointer', opacity: saving || !draft.trim() ? 0.5 : 1 }}>
            Add
          </button>
        </form>
      )}
      {loading ? (
        ...
        <div>{activeGoals.map(g => <GoalRow key={g.id} goal={g} onArchive={onArchive} onUpdate={onUpdate} />)}</div>
      )}
    </div>
  );
}
...
export default function PersonalCareDashboard() {
  ...
  const { next_session, sleep_7d, habit_ratio, personal_care_projects = [], last_therapy_entry = null, open_therapy_goals = [] } = summary || {};
    ...
    <div>
      ...
      {summary && (
        <div className="pd-week-selector">
          <button className="pd-week-btn" onClick={() => setWeekOffset(prev => prev - 1)} disabled={loading}>‹ Prev Week</button>
          ...
          <button className="pd-week-btn" onClick={() => setWeekOffset(prev => Math.min(prev + 1, 0))} disabled={loading || weekOffset >= 0}>Next Week ›</button>
        </div>
      )}
      ...
      <div style={{ marginTop: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
          <h3 style={{ margin: 0, fontSize: '14px', fontWeight: 600, color: 'var(--text-dimmed)', textTransform: 'uppercase', letterSpacing: '.06em' }}>Goals</h3>
          <Link to="/personal-care/goals" style={{ fontSize: '11px', color: 'var(--text-dimmed)', textDecoration: 'none', flexShrink: 0 }}>Goal history →</Link>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '20px' }}>
          {['personal', 'short_term', 'medium_term', 'long_term'].map(scope => (
            <GoalsWidget key={scope} scope={scope} goals={goals.filter(g => g.scope === scope)}
              onAdd={handleAddGoal} onArchive={handleArchiveGoal} onUpdate={handleUpdateGoal}
              onRename={handleRenameScope} scopeLabel={scopeLabels[scope]} loading={goalsLoading} />
          ))}
        </div>
      </div>
      ...
      <div className="dashboard-grid pd-lower-grid" style={{ marginTop: '24px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <TherapyPanel lastEntry={last_therapy_entry} openGoals={open_therapy_goals} onPrepare={() => navigate('/personal-care/journal')} />
        </div>
      </div>
    </div>
  );
